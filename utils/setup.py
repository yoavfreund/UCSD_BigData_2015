...
if len(Creds)>1:
    ...
    root_ID_index=raw_input("Which one is for launching an instance? (index)")
    EMR_ID_index=raw_input("Which one is for launching an EMR jobs? (index)")
    ID=Creds.keys()[int(ID_index)]
else:
    root_ID_index=Creds.keys()[0]

print "Creating the credentials you need to connect your laptop to and EC2 instance"
root_entry=Creds[root_ID_index]
